...
this.vowText = function (event) {
    ...
    buffer.push('   ' + {
        ...
        honored: stylize.colors.honored,
        broken:  stylize.colors.broken,
        errored: stylize.colors.errored,
        pending: stylize.colors.pending
    })[event.status]));
    ...
    if (event.status === 'broken') {
        ...
        if (event.exception.type === 'promise') {
            buffer.push('      » ' + stylize("An unexpected error was caught: " +
                           stylize(event.exception.error, 'bold'), stylize.colors[event.status]));
        } else {
            buffer.push('    ' + stylize(event.exception, stylize.colors[event.status]));
        }
    }
    return buffer.join('\n');
};
